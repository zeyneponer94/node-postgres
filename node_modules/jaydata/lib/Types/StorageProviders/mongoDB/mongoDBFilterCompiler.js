...
(0, _core.$C)('$data.storageProviders.mongoDB.mongoDBFilterCompiler', _core2.default.Expressions.EntityExpressionVisitor, null, {
    ...
    VisitParametricQueryExpression: function VisitParametricQueryExpression(expression, context) {
        ...
        if (expression.expression && expression.expression.nodeType === _core2.default.Expressions.ExpressionType.Constant) {
            context.value = expression.expression.value;
            context.queryField = context.field = _core2.default.Guid.NewGuid();
    ...
    _constExpressionFilter: function _constExpressionFilter(expression, context) {
        if (expression.left && expression.left.nodeType === _core2.default.Expressions.ExpressionType.Constant && expression.right && [_core2.default.Expressions.ExpressionType.Or, _core2.default.Expressions.ExpressionType.And].indexOf(expression.nodeType) >= 0) {
    ...
    VisitSimpleBinaryExpression: function VisitSimpleBinaryExpression(expression, context) {
        ...
        switch (expression.nodeType) {
            case _core2.default.Expressions.ExpressionType.Or:
                var orCursor = context.cursor;

                if (context.cursor instanceof Array) {
                    var or = context.unary === _core2.default.Expressions.ExpressionType.Not ? { $nor: [] } : { $or: [] };
                    context.cursor.push(or);
                    context.cursor = or[context.unary === _core2.default.Expressions.ExpressionType.Not ? '$nor' : '$or'];
                } else {
                    context.cursor[context.unary === _core2.default.Expressions.ExpressionType.Not ? '$nor' : '$or'] = [];
                    context.cursor = context.cursor[context.unary === _core2.default.Expressions.ExpressionType.Not ? '$nor' : '$or'];
                }
                if (context.unary === _core2.default.Expressions.ExpressionType.Not) context.unary = undefined;
                ...
                if (orCursor instanceof Array) {
                    for (var i = 0; i < orCursor.length; i++) {
                        ...
                        if (o.$or) {
                            if (o.$or.length == 0) {
                                ...
                                _core2.default.typeSystem.extend(o, o.$or[0]);
                                delete o.$or;
                            }
                        }
                    }
                    ...
                    if (orCursor.$or.length == 0) {
                        ...
                        _core2.default.typeSystem.extend(orCursor, orCursor.$or[0]);
                        delete orCursor.$or;
                    }
                }
                context.cursor = orCursor;
                break;
            case _core2.default.Expressions.ExpressionType.And:
                ...
                if (andCursor instanceof Array) {
                    for (var i = 0; i < andCursor.length; i++) {
                        ...
                        if (a.$and) {
                            if (a.$and.length == 0) {
                                ...
                                _core2.default.typeSystem.extend(a, a.$and[0]);
                                delete a.$and;
                            }
                        }
                    }
                    ...
                    if (andCursor.$and.length == 0) {
                        ...
                        _core2.default.typeSystem.extend(andCursor, andCursor.$and[0]);
                        delete andCursor.$and;
                    }
                }
                context.cursor = andCursor;
                break;
            case _core2.default.Expressions.ExpressionType.Equal:
            case _core2.default.Expressions.ExpressionType.EqualTyped:
                this.Visit(expression.left, context);
                this.Visit(expression.right, context);
                ...
                var v = context.value;
                if (context.entityType && context.entityType.memberDefinitions) v = this.provider.fieldConverter.toDb[_core.Container.resolveName(_core.Container.resolveType(context.entityType.memberDefinitions.getMember(context.complexType ? context.lastField : context.field).type))](v);else if (context.valueType) v = this.provider.fieldConverter.toDb[_core.Container.resolveName(_core.Container.resolveType(context.valueType))](v);
                context.value = v;
                if (context.fieldOperation) {
                    if (context.unary === _core2.default.Expressions.ExpressionType.Not) {
                    ...
                    if (context.cursor instanceof Array) {
                        var o = {};
                        o[context.queryField] = context.unary === _core2.default.Expressions.ExpressionType.Not ? { $ne: context.value } : context.value;
                        context.cursor.push(o);
                    } else context.cursor[context.queryField] = context.unary === _core2.default.Expressions.ExpressionType.Not ? { $ne: context.value } : context.value;
                    if (context.options && context.options.fields) context.options.fields[context.queryField] = 1;
                    if (context.unary === _core2.default.Expressions.ExpressionType.Not) context.unary = undefined;
                }
                break;
            case _core2.default.Expressions.ExpressionType.In:
                this.Visit(expression.left, context);
                this.Visit(expression.right, context);
                context.queryField = context.field;
                if (!context.complexType && context.entityType && context.entityType.memberDefinitions.getMember(context.field).computed) {
                ...
                if (v instanceof Array) {
                    ...
                    for (var i = 0; i < v.length; i++) {
                        if (context.entityType && context.entityType.memberDefinitions) v[i] = this.provider.fieldConverter.toDb[_core.Container.resolveName(_core.Container.resolveType(context.entityType.memberDefinitions.getMember(context.complexType ? context.lastField : context.field).type))](v[i]);else if (context.valueType) v[i] = this.provider.fieldConverter.toDb[_core.Container.resolveName(_core.Container.resolveType(context.valueType))](v[i]);
                    }
                }
                context.value = v;
                if (context.cursor instanceof Array) {
                    ...
                    if (context.entityType && context.entityType === _core2.default.Array) {
                        o[context.queryField] = context.unary === _core2.default.Expressions.ExpressionType.Not ? { $not: context.value } : context.value;
                    } else {
                        o[context.queryField][context.unary === _core2.default.Expressions.ExpressionType.Not ? '$nin' : expression.resolution.mapTo] = context.value;
                    }
                    context.cursor.push(o);
                } else {
                    context.cursor[context.queryField] = {};
                    if (context.entityType && context.entityType === _core2.default.Array) {
                        context.cursor[context.queryField] = context.unary === _core2.default.Expressions.ExpressionType.Not ? { $not: context.value } : context.value;
                    } else {
                        context.cursor[context.queryField][context.unary === _core2.default.Expressions.ExpressionType.Not ? '$nin' : expression.resolution.mapTo] = context.value;
                    }
                }
                if (context.options && context.options.fields) context.options.fields[context.queryField] = 1;
                if (context.unary === _core2.default.Expressions.ExpressionType.Not) context.unary = undefined;
    ...
    _fieldOperation: function _fieldOperation(opName, context) {
    ...
    VisitConstantExpression: function VisitConstantExpression(expression, context) {
        var valueType = _core.Container.resolveName(expression.type);
        context.valueType = valueType;
        context.value = expression.value;
        var c = Array.isArray(context.cursor) ? context.cursor.filter(function (it) {
            return typeof it[context.field] !== 'undefined';
        })[0] : context.cursor;
    ...
    _frameOperation: function _frameOperation(opDef, args, expression, context, contextinclude) {
        var self = this;
        ...
        for (var i = 0; i < args.length; i++) {
            ...
            if (arg && arg.value instanceof _core2.default.Queryable) {
                ...
                var fn = function fn(expression, context, contextinclude, indent) {
                    if (expression._expressionType === _core2.default.Expressions.AssociationInfoExpression) {
                        ...
                        if (association) {
                            var inc = context.includes.filter(function (include) {
                                return include.name == contextinclude;
                            }, this);
                            if (inc.length == 0) {
                            ...
                            association.ReferentialConstraint.forEach(function (ref) {
                                ...
                                _core.Guard.raise(new _core.Exception("The given include path is invalid: " + expression.associationInfo.FromPropertyName + ", invalid point: " + contextinclude));
                            }
                        context.defaultType = association.ToType;
                    } else if (expression._expressionType === _core2.default.Expressions.FrameOperationExpression) {
                    ...
                    } else if (expression._expressionType === _core2.default.Expressions.MemberInfoExpression) {
